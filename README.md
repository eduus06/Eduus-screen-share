...
## Ligar tudo (1 clique no Windows)

**1.** Baixe este projeto e descompacte numa pasta.

**2.** Dê **dois cliques** no arquivo:
👉 **`INICIAR DISCORD TELA.bat`**

Ele faz tudo sozinho:
- Verifica se você tem o Node.js instalado.
- Instala as dependências necessárias automaticamente na primeira vez.
- Configura suas credenciais do Discord e abre o túnel seguro.

> ⭐ **Dica:** Quando o painel abrir no seu navegador, aperte **`Ctrl + D`** para salvar nos seus **Favoritos**!

Para desligar, aperte `Ctrl + C` na janela preta ou simplesmente feche a janela.

*(Se você preferir usar o terminal manualmente, basta rodar `npm install` e depois `npm run start:fast`)*.
